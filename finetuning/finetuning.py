...
def main(args):
    ...
    method_map = ["Fine-tunning", "Fine-tunning + Detected"]
    ...
    print("Pre-training Model:" + method_map[args['method']])
    # pretraining
    for epoch in range(60):
        train(epoch, train_data, model, optimizer, args)
        sys.stdout.flush()
        scheduler.step()
    print("Re-training Model:" + method_map[args['method']])

    for epoch in range(60, args['epochs']):
        train(epoch, ctrain_data, model, optimizer, args)
        scheduler.step()
